...
describe SessionsController do
  ...
  before do 
    ...
    Citizen.stub!(:authenticate).with(@login_params[:login], @login_params[:password]).and_return(@citizen)
  end
  def do_create
        ...
        describe "my request cookie token is #{has_request_token.to_s}," do
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do 
            before do
              ...
              @ccookies.stub!(:[]=)
              @citizen.stub!(:remember_me) 
              @citizen.stub!(:refresh_token) 
              @citizen.stub!(:forget_me)
              @citizen.stub!(:remember_token).and_return(token_value) 
              @citizen.stub!(:remember_token_expires_at).and_return(token_expiry)
              @citizen.stub!(:remember_token?).and_return(has_request_token == :valid)
              if want_remember_me
            ...
            if (has_request_token == :valid)
              it 'does not make new token'   do @citizen.should_not_receive(:remember_me);   do_create end
              it 'does refresh token'        do @citizen.should_receive(:refresh_token);     do_create end 
              it "sets an auth cookie"       do do_create;  end
            else
              if want_remember_me
                it 'makes a new token'       do @citizen.should_receive(:remember_me);       do_create end 
                it "does not refresh token"  do @citizen.should_not_receive(:refresh_token); do_create end
                it "sets an auth cookie"       do do_create;  end
              else 
                it 'does not make new token' do @citizen.should_not_receive(:remember_me);   do_create end
                it 'does not refresh token'  do @citizen.should_not_receive(:refresh_token); do_create end 
                it 'kills user token'        do @citizen.should_receive(:forget_me);         do_create end 
              end
            end
          end # inner describe
        end
  ...
  describe "on failed login" do
    before do
      Citizen.should_receive(:authenticate).with(anything(), anything()).and_return(nil)
      login_as :quentin
    end
