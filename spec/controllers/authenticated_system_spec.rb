...
describe SessionsController do
  ...
  describe "logout_killing_session!" do
    ...
    it 'nils the current citizen'      do logout_killing_session!; current_citizen.should be_nil end
    it 'kills :citizen_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:citizen_id, nil).at_least(:once)
      logout_killing_session!
    end
    it 'forgets me' do    
      current_citizen.remember_me
      current_citizen.remember_token.should_not be_nil; current_citizen.remember_token_expires_at.should_not be_nil
      Citizen.find(1).remember_token.should_not be_nil; Citizen.find(1).remember_token_expires_at.should_not be_nil
      logout_killing_session!
      Citizen.find(1).remember_token.should     be_nil; Citizen.find(1).remember_token_expires_at.should     be_nil
    end
  end
  ...
  describe "logout_keeping_session!" do
    ...
    it 'nils the current citizen'      do logout_keeping_session!; current_citizen.should be_nil end
    it 'kills :citizen_id session' do
      session.stub!(:[]=)
      session.should_receive(:[]=).with(:citizen_id, nil).at_least(:once)
  ...
  describe "Logging in by cookie" do
    def set_remember_token token, time
      @citizen[:remember_token]            = token; 
      @citizen[:remember_token_expires_at] = time
      @citizen.save!
    end    
    before do 
      @citizen = Citizen.find(:first); 
